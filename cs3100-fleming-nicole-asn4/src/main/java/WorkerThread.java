...
public class WorkerThread extends TaskQueue implements Runnable
{
    //final static int NUM = 990;	// nth number of pi to print out
    //Create private placeholder of TaskQueue and ResultTable
    private TaskQueue q;
    private ResultTable r;

    //TODO need to create a constructor as specified in the instructions
    public WorkerThread(TaskQueue que, ResultTable rt)
    {
        /**
        * Instructions:
        * Create a constructor on your worker thread class that accepts a reference to the task queue and the results table,
        * storing a private reference to these objects. Then, once the class is running in a thread, it will reference those
        * objects for obtaining tasks and storing the results. This is how you will share the TaskQueue and the ResultTable
    ...
    public void run()
    {
        try {
            ...
            do {
                int key = this.q.pop();
                this.r.add(key, getDecimal(key + 1));
                this.r.printPeriod();
            } while (!q.isEmpty());
        }
        catch(Exception e){
    ...
    //TODO Compute that number of pi
    /**
